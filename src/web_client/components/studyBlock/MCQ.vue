...
<div class="border rounded-lg mt-2 mb-4">
    ...
    <div class="p-4">
        ...
        <p>{{contentBlock.mcq_question_text}}</p>
    </div>
    <!-- End Inner Question -->
    <hr>
    <!-- Inner Answers -->
...
function consumeBlock() {
    if (currentAnswer.value != null) {
        ...
        testAnswer(currentAnswer.value).then(isCorrect => {
            if (isCorrect) {
                answers.push({answer: currentAnswer.value, correct: true, attempt: answers.length})
                ...
                answers.push({answer: currentAnswer.value, correct: false, attempt: answers.length})
