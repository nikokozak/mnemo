...
<div class="border rounded-lg mt-2 mb-4">
    ...
    <template v-for="brick in contentBlock.static_content">
    ...
    <div class="flex justify-around">
        <button @click="addText" class="flex text-xs border rounded-lg py-2 px-4">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4 mr-2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25H12" />
            </svg>
            <p>Add Text</p>
        </button>
        <button @click="addImage" class="flex text-xs border rounded-lg py-2 px-4">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4 mr-2">
...
function deleteContentBlock() {
    $fetch(`${config.public.baseURL}/api/content_block/${contentBlock.value.id}`, {
        method: 'DELETE'
    }).then(response => {
        emit('delete');
    })
