defmodule Mnemo.Engines.Block do
  ...
  def test_block_type("fibq", %Block{} = block, answers) do
    # answers is expected to be a comma-separated string for storage reasons, i.e. "tree, sky"
    ...
    graded_answers =
      ...
      |> Enum.with_index()
      |> Enum.map(fn {answer, input_idx} ->
        ref = Enum.find(actual_answers, fn aa -> aa["input_idx"] == input_idx end)
    ...
    IO.inspect(graded_answers, label: "graded answers")
